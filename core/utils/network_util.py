...
class FeatureVolDecoder(nn.Module):
    r""" Convolutional 3D volume decoder."""

    def __init__(self, input_ch=182, volume_size=128, voxel_channels=4):
        r""" 
            Args:
                embedding_size: integer
                volume_size: integer
                voxel_channels: integer
        """    
        super(FeatureVolDecoder, self).__init__()

        self.act_fn = nn.GELU()
        self.conv1 = nn.Conv2d(input_ch, 256, 3, 1) # 128x256x256
        self.conv1_1 = nn.Conv2d(256, 256, 3, 1)
        self.conv1_ds = nn.Conv2d(256, 256, 3, 2)

        self.conv2 = nn.Conv2d(512, 512, 3, 1) # 64x128x128
        self.conv2_1 = nn.Conv2d(512, 512, 3, 1)
        self.conv2_ds = nn.Conv2d(512, 256, 3, 2)

        self.conv3 = nn.Conv2d(256, 256, 3, 1) # 32x64x64
        self.conv3_1 = nn.Conv2d(128, 128, 3, 1)
        self.conv3_ds = nn.Conv2d(128, 32, 3, 2) # 32x32x32
...
class MotionBasisComputer(nn.Module):
    ...
    def _construct_G(self, R_mtx, T):
        r''' Tile rotation matrix and translation vector to build a 4x4 matrix.
    ...
    def forward(self, dst_Rs, dst_Ts, cnl_gtfms, forward_kinematics=False):
        r"""
        Args:
        ...
        if forward_kinematics:
            ...
            f_mtx = torch.matmul(cnl_gtfms, inv_dst_gtfms) # observation to canonical space trasform based on human pose
            f_mtx = f_mtx.view(-1, self.total_bones, 4, 4)
