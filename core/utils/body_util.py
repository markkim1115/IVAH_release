...
def _deform_gaussian_volume(
    ...
    SIGMA = R.dot(S).dot(S).dot(R.T) # R@S^2@R^T
    ...
    dist = np.einsum('abci, abci->abc', np.einsum('abci, ij->abcj', grid, SIGMA), grid) # (x-mu)^T@SIGMA@(x-mu), This means the distance from the center of Gaussian to the grid

    return np.exp(-1 * dist) # lastly, apply Gaussian function
...
def approx_gaussian_bone_volumes(
    ...
    Returns:
        - Array (Total_Joints + 1, grid_size, grid_size, grid_size)
    """
    ...
    for joint_idx in range(0, total_joints):
        ...
        for bone_idx, parent_idx in SMPL_PARENT.items():
            if joint_idx != parent_idx: # Find the parent joint
                continue

            S = _std_to_scale_mtx(BONE_STDS * 2.) # In scale element, if larger std, smaller scale value otherwise, larger scale value
            if joint_idx in TORSO_JOINTS: # If the joint is torso joint, scale down the volume, equivalent to scale up the std
                S[0][0] *= 1/1.5
                S[2][2] *= 1/1.5

            start_joint = tpose_joints[SMPL_PARENT[bone_idx]] # Find the start joint, which is the parent joint
            end_joint = tpose_joints[bone_idx] # Find the end joint, which is the current joint
            target_bone = (end_joint - start_joint)[None, :] # Vector direction is from child joint to parent joint

            # Get rotation matrix from calibrated bone to target bone. Simply, this is global rotation matrix of the bone
...
def query_dst_skeleton(mesh_infos, frame_name):
    return {
        'poses': mesh_infos[frame_name]['poses'].astype('float32'),
        'dst_tpose_joints': mesh_infos[frame_name]['tpose_joints'].astype('float32'),
        'bbox': mesh_infos[frame_name]['bbox'].copy(),
        'Rh': mesh_infos[frame_name]['Rh'].astype('float32'),
        'Th': mesh_infos[frame_name]['Th'].astype('float32')
    }

def construct_cnl_body_data(cnl_joints, keyfilter):
    # Set default pose as to be t-pose
    dst_poses = np.zeros(72, dtype='float32')
    
    dst_skel_joints = cnl_joints
    out = {}
    if 'motion_bases' in keyfilter:
            dst_Rs, dst_Ts = body_pose_to_body_RTs(
                    dst_poses, dst_skel_joints
                )
            cnl_gtfms = get_canonical_global_tfms(cnl_joints)
            out.update({
                'dst_Rs': dst_Rs,
                'dst_Ts': dst_Ts,
                'cnl_gtfms': cnl_gtfms
    ...
    if 'dst_posevec_69' in keyfilter:
    ...
    with open(path) as file:
        while True:
            ...
            if strs[0] == "f":
                ...
                faces.append(face)
                faces_uv.append(texcoord)
                vt_to_v[texcoord[0]] = face[0]
                vt_to_v[texcoord[1]] = face[1]
                vt_to_v[texcoord[2]] = face[2]

                v_to_vt[face[0]] = texcoord[0]
                v_to_vt[face[1]] = texcoord[1]
                v_to_vt[face[2]] = texcoord[2]
        ...
        model['uv'] = uv
        model['faces_uv'] = np.array(faces_uv)
        model['vt_to_v'] = vt_to_v
...
def generate_2d_heatmap(joints, image_res):
    ...
    :param image_res: (w, h)
    :return: 2D heatmap (24, w, h)
    """
    joints = joints[:, :2]
    heatmap = np.zeros((joints.shape[0], *image_res))
    ...
    for idx, pt in enumerate(joints):
        ...
        heatmap[idx, cc:dd, aa:bb] = np.maximum(heatmap[idx, cc:dd, aa:bb], gussian_kernel[c:d, a:b])
