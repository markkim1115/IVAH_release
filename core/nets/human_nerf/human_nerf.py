...
class HumanNeRF(nn.Module):
    def __init__(self):
        ...
        if cfg.append_rgb:
            get_embedder = load_positional_embedder(cfg.embedder.module)
            view_embed_fn, _ = get_embedder(4,0)
    ...
    def _render_rays(
        ...
        if cfg.back_net.back_net_on or cfg.use_back_img_gt:
            back_img = kwargs['back_img'] if cfg.db not in ['humman', 'thuman1', 'zju_mocap'] else torch.flip(kwargs['back_img'], dims=[-1])
            back_fmap = kwargs['back_fmap'] if cfg.db not in ['humman', 'thuman1', 'zju_mocap'] else torch.flip(kwargs['back_fmap'], dims=[-1])
