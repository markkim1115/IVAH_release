...
data_root = '/media/cv1/data/THuman2.0/meshes'
smpl_root = '/media/cv1/data/THuman2.0/THuman2.0_smpl'
subjects = sorted(os.listdir(data_root))

for subject_idx in range(len(subjects)):
    subject = subjects[subject_idx]

    mesh_path = os.path.join(data_root, subject, subject+'.obj')

    smpl_path = os.path.join(smpl_root, f'{subject}_smpl.pkl')
    
    with open(smpl_path, 'rb') as f:
        smpl_data = pickle.load(f)
    global_orient = smpl_data['global_orient'] # (1,3)
    body_pose = smpl_data['body_pose'] # (1,23,3)
    body_pose = body_pose.reshape(1,-1) # (1,69)
    betas = smpl_data['betas'] # (1,10)
    transl = smpl_data['transl'] # (1,3)
    scale = smpl_data['scale'] # (1,)

    smpl_out = get_smpl_from_numpy_input(smpl_model, None, body_pose, betas, None, 'cuda')
    smpl_verts = smpl_out.vertices.detach().cpu().numpy()[0]
    smpl_joints = smpl_out.joints.detach().cpu().numpy()[0]
    smpl_root_joint = smpl_joints[0].reshape(1,3)
    global_rot_mat = cv2.Rodrigues(global_orient)[0]
    Th = smpl_root_joint - smpl_root_joint@global_rot_mat.T + np.array(transl).astype(np.float32) / scale
    smpl_verts = smpl_verts@global_rot_mat.T + Th 
    ...
    faces = mesh.faces
    vertex_colors = mesh.visual.to_color().vertex_colors[...,:3]
    ...
    save_root_1 = os.path.join('dataset/thuman2', subject)
