...
class Camera:
    def __init__(self, width=512, height=512, focal=5000, focal_x=None, focal_y=None, cx=None, cy=None, near=0.1, far=40):
        # Focal Length
        # equivalent 50mm
        # focal = np.sqrt(width * width + height * height)
        self.focal_x = focal if focal_x is None else focal_x
        self.focal_y = focal if focal_y is None else focal_y
        # Principal Point Offset
        self.principal_x = width / 2 if cx is None else cx
        self.principal_y = height / 2 if cy is None else cy
        # Axis Skew
        self.skew = 0
        # Image Size
        self.width = width
