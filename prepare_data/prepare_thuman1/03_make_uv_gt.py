...
class UVMapGenerator():
    ...
    def complete_uv_map_from_sparse_colored_smpl_vertex(self, sparse_vertex_colors):
        ...
        uv, _ = self.get_UV_map(sparse_vertex_colors, dilate=False)
...
data_root = '/media/cv1/T7/THuman1_raw/dataset'
mps_data_root = '/media/cv1/T7/THuman_MPS_NeRF/nerf_data_'
matching_list_root = '/media/cv1/T7/THuman1_raw/matching_with_mps_nerf'
subjects = sorted([x for x in os.listdir(mps_data_root) if os.path.isdir(os.path.join(mps_data_root, x))])
for subject_idx in range(len(subjects)):
    
    subject = subjects[subject_idx]

    pose_name_matching_file = os.path.join(matching_list_root, subject+'.txt')
    ...
    with open(pose_name_matching_file, 'r') as f:
        ...
        for line in lines:
            ...
            pose_name_matching_dict[mps_nerf_pose_name] = pose_name

    mps_subject_frames = os.listdir(os.path.join(mps_data_root, subject, 'new_params_neutral'))
    mps_subject_frames = sorted(mps_subject_frames)
    pidx = 5
    mps_pose_path = mps_subject_frames[pidx]
    
    mps_pose_name = mps_pose_path[:-4]
    pose_name = pose_name_matching_dict[mps_pose_name]
    mesh_path = os.path.join(data_root, subject, pose_name, 'mesh.obj')
    smpl_path = os.path.join(data_root, subject, pose_name, 'smpl.obj')

    # Load scan mesh
    mesh = trimesh.load_mesh(mesh_path)
    # get scan mesh's vertex colors
    vertices = np.array(mesh.vertices)
    faces = np.array(mesh.faces)
    vertex_colors = np.array(mesh.visual.vertex_colors[...,:3])

    # Load SMPL mesh
    smpl_mesh = trimesh.load_mesh(smpl_path)
    smpl_verts = np.array(smpl_mesh.vertices)
    smpl_faces = np.array(smpl_mesh.faces)
    
    vertices = torch.from_numpy(vertices).float().cuda()[None]
    smpl_verts = torch.from_numpy(smpl_verts).float().cuda()[None]

    dist, idx, nn = knn_points(smpl_verts, vertices, K=1)
    idx = idx.reshape(-1).cpu().numpy()
    smpl_colors = vertex_colors[idx]
    save_root_1 = os.path.join(mps_data_root, subject)
    frame_number = int(mps_pose_name)
    save_path_1 = os.path.join(save_root_1, 'uvmap_gt.png')

    uv = gnr.complete_uv_map_from_sparse_colored_smpl_vertex(smpl_colors)
    Image.fromarray(uv).save(save_path_1)
