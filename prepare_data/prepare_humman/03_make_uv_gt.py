...
data_root = 'dataset/humman'
subjects = sorted([x for x in os.listdir(data_root) if os.path.isdir(os.path.join(data_root, x))])

for subject_idx in range(len(subjects)):
    subject = subjects[subject_idx]
    textured_mesh_path = os.path.join(data_root, subject, 'textured_meshes')
    smpl_params_path = os.path.join(data_root, subject, 'smpl_params')

    frame_names = sorted([x[:-4] for x in os.listdir(smpl_params_path) if x.endswith('.npz')])
    frame_name = frame_names[0]

    # Load SMPL mesh
    smpl_param = np.load(os.path.join(smpl_params_path, frame_name + '.npz'))
    global_orient = smpl_param['global_orient'][None].reshape(1,3)
    body_pose = smpl_param['body_pose'][None].reshape(1,69)
    betas = smpl_param['betas'][None].reshape(1,10)
    transl = smpl_param['transl'][None].reshape(1,3)
    smpl_out = get_smpl_from_numpy_input(smpl_model, global_orient, body_pose, betas, transl, 'cuda')
    smpl_verts = smpl_out.vertices[0].cpu().numpy()
    smpl_faces = smpl_face

    # Load scan mesh
    mesh = trimesh.load_mesh(os.path.join(textured_mesh_path, frame_name + '.obj'))

    verts = mesh.vertices
    faces = mesh.faces
    vertex_colors = mesh.visual.to_color().vertex_colors[...,:3]

    verts = torch.from_numpy(verts).float().cuda()[None]
    smpl_verts = torch.from_numpy(smpl_verts).float().cuda()[None]

    dist, idx, nn = knn_points(smpl_verts, verts, K=1)
    idx = idx.reshape(-1).cpu().numpy()
    smpl_colors = vertex_colors[idx]

    save_root = os.path.join(data_root, subject, 'uvmap_gt')
    os.makedirs(save_root, exist_ok=True)
    save_path = os.path.join(save_root, 'uvmap_gt.png')

    uv = gnr.complete_uv_map_from_sparse_colored_smpl_vertex(smpl_colors)
    Image.fromarray(uv).save(save_path)
